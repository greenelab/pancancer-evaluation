...
# Script to run plot_ridge_results.ipynb for several different
...
for gene in "${genes[@]}"; do
    ...
    cmd="papermill plot_ridge_results.ipynb "
    cmd+="${papermill_output_dir}/plot_ridge_results.run.ipynb "
    cmd+="-k ${kernel_name} "
    cmd+="-p gene ${gene}"
    echo "Running: $cmd"
    eval $cmd
